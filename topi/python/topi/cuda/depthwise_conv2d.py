...
def schedule_depthwise_conv2d_nhwc(cfg, outs):
    ...
    def _callback(op):
        if op.tag == 'depthwise_conv2d_nhwc':
            ...
            kernel_scope, n = s[output].split(n, nparts=1)
            by = s[output].fuse(n, by)
            s[output].bind(by, te.thread_axis("blockIdx.z"))
            s[output].bind(bx, te.thread_axis("blockIdx.y"))
            s[output].bind(bf, te.thread_axis("blockIdx.x"))
            s[output].bind(vy, te.thread_axis("vthread"))
            s[output].bind(vx, te.thread_axis("vthread"))
            s[output].bind(vf, te.thread_axis("vthread"))
            s[output].bind(ty, te.thread_axis("threadIdx.z"))
            s[output].bind(tx, te.thread_axis("threadIdx.y"))
            ...
            # cooperative fetching
            s[AA].compute_at(s[output], bf)
            s[WW].compute_at(s[output], bf)
            s[AL].compute_at(s[output], tf)
            s[WL].compute_at(s[output], tf)

            for load in [AA, WW]:
                fused = s[load].fuse(*list(s[load].op.axis))
                fused, tf = s[load].split(fused, cfg["tile_f"].size[2])
                fused, tx = s[load].split(fused, cfg["tile_x"].size[2])
                fused, ty = s[load].split(fused, cfg["tile_y"].size[2])
                s[load].bind(ty, te.thread_axis("threadIdx.z"))
                s[load].bind(tx, te.thread_axis("threadIdx.y"))
                s[load].bind(tf, te.thread_axis("threadIdx.x"))
...
def schedule_depthwise_conv2d_nhwc_old(outs):
    """Schedule for depthwise_conv2d nhwc forward.
    Parameters
    ----------
    outs: Array of Tensor
